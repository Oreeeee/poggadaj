...
func (p *GG_Notify_Reply) Serialize(stream *utils.IOStream) {
	...
	stream.WriteU16(p.Unknown1)
	stream.WriteU8(p.DescriptionLen)
	stream.Write(p.Description)
}

func (p *GG_Notify_Reply) Deserialize(stream *utils.IOStream) {}

func (p *GG_Notify_Reply) PrettyPrint() []string {
	s := []string{
